...
- **Release stamp stays `skills-v0.1.0`** on every skill; `release.json` is not edited. `skills.lock.json` is regenerated by `npm run release`, never by hand.
- **Catalog is generated.** After any `SKILL.md` change, run `npm run catalog` and commit its output. Never hand-edit `catalog.json` or `CATALOG.md`.
...
/** A temp repo root with the given concern files present. */
...
git add docs/profile.md skills/fieldnote-testing/SKILL.md cli/src/profile.test.ts catalog.json CATALOG.md
git commit -m "docs: supersede the profile's Architecture section with concerns/shared.md"
```

(`fieldnote-testing`'s description is unchanged, so `npm run catalog` should produce no diff. Commit the catalog files only if it does.)
...
npm run validate && npm run check:decoupling && npm run catalog
```
Expected: validate reports 9 skills valid; decoupling reports no violations; `catalog.json` and `CATALOG.md` gain the new entry.
...
```bash
npm run validate && npm run check:decoupling
git add skills/fieldnote-do-work README.md docs/the-loop.md catalog.json CATALOG.md
...
```bash
cd cli && npm test && npm run typecheck && cd ..
git add skills/fieldnote-setup-profile catalog.json CATALOG.md
